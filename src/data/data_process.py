            ...
            if file.startswith("acs"):
                ...
                for sex in [0, 1, 3]:
                    for race in ['RACAIAN', 'RACASN', 'RACBLK', 'RACNUM', 'RACWHT', 'RACSOR', 'HISP', "ALL"]:
                        ...
                        df = df.group_by("year", "state", "PUMA").agg(pl.col("PWGTP", "total_time").sum())
                        df = df.select("year", "state", "PUMA", (pl.col("total_time") / pl.col("PWGTP")).alias("avg_time"))
                        df = df.with_columns(pl.col("year").cast(pl.String).str.to_date("%Y"),
                                            sex=sex, race=pl.lit(race))
